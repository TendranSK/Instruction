...
Появится помощник, с помощью которого вы можете оценить какую информацию заменит то или иное дествие; Вы сможете сохранить текущее состояние, изменное состояние, либо сохранить оба состояния. После выбора действия произведите `коммит` слияния.
...
## <center>12. Работа с удалёнными репозиториями </center>

Чтобы работать с удалёнными репозиториями, нужно:
1. Создать аккаунт на **GitHub**.
2. Создать удалённый репозиторий на **GitHub**
3. Создать локальный репозиторий.
4. Cвязать удалённый репозиторий с локальным репозиторием.
5. Использовать команду `git remote add <псевдоним_подключения> <ссылка_удалённого_репозитория>` в терминале на локальном репозитории.

**Обязательно убедитесь в том, что ссылка, которая указывает на ваш удаленный репозиторий была `правильной`, иначе вы получите `ошибку` и не сможете рабоать с удаленным репозиторием.**

<center>

![gitRA](gitRemoteAdd.png)

</center>

Если вы допустили ошибку во время подключения локального репозитория к удаленному, используйте команду `git remote remove <псевдоним_подключения>` для удаления привязанного подключения.

Чтобы "вытянуть" файлы проекта из удаленного репозитория, используйте команду `git pull <псевдоним_подключения> <название_ветки_удаленного_репозитория>`.

Если в проекте удаленного репозитория есть несвязанные ветки с вашим локальным репозиторием, вам выдаст ошибку `<fatal: refusing to merge unrelated histories>`. Чтобы исправить такую ошибку, используйте команду `git pull --allow-unrelated-histories <псевдоним_подключения> <название_ветки_удаленного_репозитория>`.

<center>

![gitPMC](gitPullMergeConf.png)

</center>

Чтобы сохранить внесенные изменения в проект из локального репозитория в удаленный, используйте команду ` git push --set-upstream <псевдоним_подключения> <название_ветки_локального_репозитория>` (чтобы подключить поток). Далее можно использовать команду `git push`, т.к. уже создан поток.
___
### <center>**Pull Request в GitHub**</center>
...
## <center>13. Перечень команд </center>
...
---
###### Описанание Pull Request было взято с сайта git-scm.com | Учебник по Git'у
